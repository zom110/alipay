 ...
 */
@Service
public class SysPayCacheServiceImpl implements SysPayCacheService {

	private static final Logger logger = LoggerFactory.getLogger(SysPayCacheServiceImpl.class);

	/**
	 *  支付编号关联支付ID缓存,
	 */
	private static final String PAYCODERELPAYIDCACHE_KEYPRE = "spct_syspcdRelId:"; 
	 ...
	 */
	private static final String PAYCODEPOOLCACHE_KEYPRE = "spct_syspcd4CrtPool:"; 
	 ...
	 *  支付创建中的缓存.
	 */
	private static final String PAYCREATELOCKCACHE_KEYPRE = "spct_syspcdInCreate:"; 
     ...
     * 用户支付配置默认值
     */
    private static final String CACHEKEY_SYSPAYDFTCFG = "pct_syspaydftcfg";

    /**
     * 用户支付默认值缓存时间
     */
    private static final int CACHEKEY_SYSPAYDFTCFG_EXPSEC = 60;
...
//	@Resource(name=PaycenterConstants.SERVICENAME_BASEBYTECACHE)
    ...
    @Override
    public SyspayDftCfgInf getCachedSyspayDftCfgInf() {
        String cacheKey = CACHEKEY_SYSPAYDFTCFG ;
        byte[] byteArray = baseBytesCacheService.getByteArray(cacheKey);
        if(byteArray == null || byteArray.length < 5  ) {
            return null ;
        }else {
            return JSONObject.parseObject(byteArray, SyspayDftCfgInf.class, Feature.IgnoreNotMatch);
        }
    }

    @Override
    public void cacheSyspayDftCfgInf(SyspayDftCfgInf cachedInf) {
        String cacheKey = CACHEKEY_SYSPAYDFTCFG ;
        byte[] jsonBytes = JSONObject.toJSONBytes(cachedInf, SerializerFeature.IgnoreErrorGetter);
        if( jsonBytes != null && jsonBytes.length > 5 ) {
            baseBytesCacheService.putAndExpireByteArray(cacheKey, jsonBytes, CACHEKEY_SYSPAYDFTCFG_EXPSEC);
        }
    }
...
//	 */
//	@Override
//	public void incUserCudayPaycreatCnt(UserPayChanelEnum chnlEnum , Long userId) {
...
//
//		// 秒数 
//		long expSecond = ( cld.getTimeInMillis() - currTime.getTime() )/1000 ; 
//		if(expSecond < 1 ){ 
//		    // 至少缓存1秒 
//			expSecond = 1 ;
//		}
...
//	 */
//	@Override
//	public int getUserCudayPaycreateCnt(UserPayChanelEnum chnlEnum , Long userId) {
//
//		String cacheKey = USRCUDAYCREATEPAYCNTCACHE_KEYPRE + chnlEnum.getCode() + ":" + userId ; 
//
//		byte[] byteArray = baseBytesCacheService.getByteArray(cacheKey);
